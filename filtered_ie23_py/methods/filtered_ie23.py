...
def filtered_ie23(f_ode, t_range, y_init, dt, TOL, max_steps):
    ...
    while t[k] < t_range[1]:
        ...
        while True:
            ...
            temp_yk, kappa_nm1 = _IE_adaptive_pre_filter(
                y[k, :], y[k - 1, :], y[k - 2, :], kn, knm1, knm2
            )
            y[k + 1, :] = fsolve(
                _ier_updated, y[k + 1, :], args=(f_ode, kn, temp_yk, t[k + 1])
            )
            ynp1_order2 = y[k + 1, 0]
            y[k + 1, :] = _IE_adaptive_post_filter(
                y[k + 1, :], y[k, :], y[k - 1, :], kappa_nm1, kn, knm1, knm2, knm3
            )
            ynp1_order3 = y[k + 1, 0]
...
def _IE_adaptive_pre_filter(y_n, y_nm1, y_nm2, kn, knm1, knm2):
    kappa_nm1 = (
        (2 * knm2 / (knm1 + knm2)) * y_n
        - 2 * y_nm1
        + (2 * knm1 / (knm1 + knm2)) * y_nm2
    )
...
def _IE_adaptive_post_filter(y_np1, y_n, y_nm1, kappa_nm1, kn, knm1, knm2, knm3):
    kappa_n = (
        (2 * knm1 / (kn + knm1)) * y_np1 - 2 * y_n + (2 * kn / (kn + knm1)) * y_nm1
    )
    post_filter = -(kn**2 * (knm1 + kn) * (knm2 + 2 * (knm1 + kn))) / (
        ...
        * (
            2 * knm2**2 * (knm1 + kn)
            + 3 * knm3 * (knm2 - kn) * (knm1 + kn)
            - 2 * knm1 * kn * (knm1 + kn)
            + knm2 * (knm1**2 - 5 * knm1 * kn - 7 * kn**2)
        )
    )
